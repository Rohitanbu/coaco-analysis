...
// Batch Builder Logic
function createSampleRow() { console.log("createSampleRow called!");
...
function mapApiResponse(data) {
    ...
    return {
        ...
        other_possibilities: data.probabilities.slice(1).map(item => ({
            label: item.label,
...
function populateResultBlock(mapped, blockNode, fileObj) {
    ...
    if (lower.includes('ripe') && !lower.includes('over') && !lower.includes('under') && !lower.includes('un')) {
        labelEl.classList.add('ripe');
    } else if (lower.includes('un') || lower.includes('under')) {
        labelEl.classList.add('unripe');
    } else if (lower.includes('over')) {
        labelEl.classList.add('overripe');
    }
    ...
    if (mapped.confidence < CONFIDENCE_THRESHOLD) {
        ...
        listEl.innerHTML = mapped.other_possibilities.map(item => {
            const cls = colorMap[item.label] || '';
            return `
                <div class="possibility-row">
                    <div class="possibility-label">
                        <span>${item.label}</span>
                        <span>${item.confidence}%</span>
                    </div>
                    <div class="possibility-track">
                        <div class="possibility-fill ${cls}" style="width:${item.confidence}%"></div>
                    </div>
                </div>`;
        }).join('');
    } else {
